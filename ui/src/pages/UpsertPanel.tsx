...
import { Button, ErrorNote, Field, Panel } from "../components";
...
function parseRecords(text: string, dimension: number): Parsed {
  ...
  for (const [i, rec] of records.entries()) {
    ...
    if (values.length !== dimension) return { ok: false, message: `${where} (${id}) has ${values.length} values; this index is ${dimension}-d.` };
...
function example(dimension: number) {
  ...
  const records = genres.map((genre, i) => ({
    id: `example-${i + 1}`,
    values: randomUnitVector(dimension),
    metadata: { genre, year: 2021 + i, tags: i ? ["new"] : ["classic", "award"] },
  }));
  return `[\n${records.map((r) => `  ${JSON.stringify(r)}`).join(",\n")}\n]`;
}

export default function UpsertPanel({ info, namespaces, onDone }: {
  info: IndexInfo;
  namespaces: string[];
  onDone: () => void;
}) {
  ...
  async function submit() {
    ...
    try {
      ...
      setDone(`Upserted ${fmtInt(total)} ${total === 1 ? "record" : "records"} into ${namespace ? `“${namespace}”` : "the default namespace"}.`);
      onDone();
    } catch (err) {
  ...
  return (
    <Panel title="Upsert" note="new ids are inserted; existing ids are replaced"
      actions={<Button onClick={() => setText(example(info.dimension))}>Insert example</Button>}>
      <div className="form-stack">
        <Field label="Records" htmlFor="u-records"
          ...
          <textarea id="u-records" rows={14} spellCheck={false} value={text} onChange={(e) => setText(e.target.value)}
            placeholder={'[\n  {"id": "doc-1", "values": [0.1, …], "metadata": {"genre": "drama"}}\n]'} />
        </Field>
        <div className="row">
          <Field label="Namespace" htmlFor="u-ns" hint="Pick an existing one or type a new name.">
            <input id="u-ns" className="mono" list="u-ns-list" value={namespace} placeholder="(default)" onChange={(e) => setNamespace(e.target.value)} />
            <datalist id="u-ns-list">{namespaces.filter(Boolean).map((ns) => <option key={ns} value={ns} />)}</datalist>
          </Field>
        </div>
        <ErrorNote error={error} />
        {done && <div className="ok-note" role="status">{done}</div>}
        <div className="row-end">
          <Button variant="primary" onClick={() => void submit()} disabled={busy || !parsed.ok}>{busy ? "Upserting…" : "Upsert"}</Button>
        </div>
      </div>
    </Panel>
  );
